...
// 	// 	cout << "Image " << i << " label: " << images[i].label << " ("
// << images[i].class_name << ")\n";
// 	// 	cout << "Pixel (0,0) R: " << (int)images[i].data[0][0][0] << "
// G: " << (int)images[i].data[1][0][0] << " B: " <<
// (int)images[i].data[2][0][0] << "\n";
// 	// }
...
int main() {
  ...
  // std::vector<Image> train_images = loadCIFARBatch(train_path, 1000);  //
  // Training
  ...
  model.train(train_images,
              10);  // assume you loaded training as `train_images`
  ...
  std::cout << "LogReg Train Accuracy: "
            << (float)correct / train_images.size() * 100 << "%\n";
  ...
  for (const auto& img : test_images) {
    ...
    if (pred == img.label) {
      correct++;
      std::cout << "Prediction: " << pred << " Actual: " << img.label
                << "		CORRECT!!\n";
    }
  }
  std::cout << "LogReg Test Accuracy: "
            << (float)correct / test_images.size() * 100 << "%\n";
  ...
  for (const auto& img : train_images) {
    ...
    int pred = argmax(out);
    if (pred == img.label) cnn_correct++;
  }
  std::cout << "CNN Train Accuracy (multiclass): "
            << (float)cnn_correct / train_images.size() * 100 << "%\n";
  ...
  std::cout << "CNN Test Accuracy (multiclass): "
            << (float)cnn_correct / test_images.size() * 100 << "%\n";
