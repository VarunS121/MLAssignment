...
std::vector<Image> loadCIFARBatch(const std::string& filename, int max_images) {
    ...
    std::vector<std::string> class_names = loadLabelNames("./cifar-10-batches-bin/batches.meta.txt");
...
std::vector<std::vector<std::vector<float>>> Image::to3D() const {
    std::vector<std::vector<std::vector<float>>> result(3, std::vector<std::vector<float>>(32, std::vector<float>(32)));
